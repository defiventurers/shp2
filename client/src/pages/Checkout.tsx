...
import { z } from "zod";import { useState } from "react";
import { useLocation } from "wouter";
import { useMutation, useQuery } from "@tanstack/react-query";
import { MapPin, Truck, Store, AlertTriangle, Check } from "lucide-react";
import { useForm } from "react-hook-form";
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <Card className="p-4">
              ...
              >
                ...
                <p className="text-xs text-muted-foreground ml-7">{PICKUP_ADDRESS}</p>

                <Label className="flex gap-3 items-start mt-4">
                  <RadioGroupItem value="delivery" />
                  <Truck /> Home Delivery (₹{DELIVERY_FEE})
                </Label>
              </RadioGroup>
            ...
            <Card className="p-4">
              {items.map((i) => (
                <div key={i.medicine.id} className="flex justify-between text-sm mb-1">
                  <span>{i.medicine.name} x{i.quantity}</span>
                  <span>₹{Number(i.medicine.price) * i.quantity}</span>
                </div>
              ))}
